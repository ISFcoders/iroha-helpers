...
function getEngineResponse(queryOptions, params) {
    return sendQuery(queryOptions, queryHelper_1["default"].addQuery(queryHelper_1["default"].emptyQuery(), 'getEngineResponse', validation_1["default"](params, ['txHash'])), function (resolve, reject, responseName, response) {
        if (responseName !== 'ENGINE_RESPONSE') {
            var error = JSON.stringify(response.toObject().errorResponse);
            return reject(new Error("Query response error: expected=ENGINE_RESPONSE, actual=" + responseName + "\nReason: " + error));
        }
        var info = response.getEngineResponse().toObject().engineResponseRecordsList;
        resolve(info);
    });
}
/**
